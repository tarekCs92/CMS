...
namespace CaloriCms.Controllers
{
    ...
    public class NewsController : Controller
    {
        ...
        {
            ViewBag.Section = "المعرض";
            using (var db = new PersonalityDBEntities())
            {
                int pageSize = 5;
                var data = db.GallaryTBs.Where(i => i.SectionId == 1).ToList();
        ...
        public ActionResult Create(GallaryTB modelTb, HttpPostedFileBase file)
        {
            string abPath = "";
            using (var db = new PersonalityDBEntities())
            {
        ...
        public ActionResult Edit(int id)
        {
            ...
            using (var db = new PersonalityDBEntities())
            {
                var data = db.GallaryTBs.Find(id);
                return View(data);
            }

        }
        [HttpPost, ValidateInput(false)]
        public ActionResult Edit(GallaryTB modelTb, HttpPostedFileBase file)
        {
            string abPath = "";
            using (var db = new PersonalityDBEntities())
            {
