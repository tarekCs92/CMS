...
namespace CaloriCms.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public ActionResult Index(string searchString, int page = 1)
        {
            ...
            {
                int pageSize = 5;
                var data = db.BlogTBs.Where(i => i.SectionId == 1).ToList();
        ...
        public ActionResult Create(BlogTB modelTb, HttpPostedFileBase file)
        {
            string abPath = "";
            using (var db = new PersonalityDBEntities())
            {
        ...
        public ActionResult Edit(int id)
        {
            ...
            using (var db = new PersonalityDBEntities())
            {
                var data = db.BlogTBs.Find(id);
                return View(data);
            }

        }
        [HttpPost, ValidateInput(false)]
        public ActionResult Edit(BlogTB modelTb, HttpPostedFileBase file)
        {
            string abPath = "";
            using (var db = new PersonalityDBEntities())
            {
        ...
        {
            ViewBag.Section = "النص الجانبي";
            using (var db = new PersonalityDBEntities())
            {
                var data = db.BlogTBs.FirstOrDefault(x => x.SectionId == 2);
                return View(data);
            }

        }
        [HttpPost, ValidateInput(false)]
        public ActionResult Section(BlogTB modelTb)
        {
            using (var db = new PersonalityDBEntities())
            {
                db.Entry(modelTb).State = EntityState.Modified;
